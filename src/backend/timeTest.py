...
TORCHSCRIPT_MODEL_PATH = Path(
    r"C:\Users\ser9e\New_classify\src\backend\models\bert\bert_ts.pt"
)
...
print(
    f"HF model inference time (50 runs): {hf_time:.4f} sec, avg: {hf_time/50:.4f} sec/run"
)
print(
    f"TorchScript inference time (50 runs): {ts_time:.4f} sec, avg: {ts_time/50:.4f} sec/run"
)
print(
    f"ONNX inference time (50 runs): {onnx_time:.4f} sec, avg: {onnx_time/50:.4f} sec/run"
)
